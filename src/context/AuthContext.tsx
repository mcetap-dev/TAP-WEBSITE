...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchProfile = async (userId: string) => {
    try {
      // Load saved local profile if present
      let localProfile: Record<string, any> = {};
      try {
        const stored = localStorage.getItem(`pc_profile_${userId}`);
        if (stored) {
          localProfile = JSON.parse(stored);
        }
      } catch (err) {
        console.warn('Error reading local profile storage:', err);
      }
      ...
      const dbData = data || {};
      const merged: Record<string, any> = { ...localProfile, ...dbData };

      // Ensure null or missing DB properties do not wipe out valid local profile edits
